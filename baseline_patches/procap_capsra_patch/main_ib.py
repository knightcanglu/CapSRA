...
import config
from train_ib import train_for_epoch
...
if __name__ == "__main__":
    ...
    model = getattr(pbm, "build_baseline")(
        ...
        ib_hidden_dim=getattr(opt, "IB_HIDDEN_DIM", None),
        ib_beta=getattr(opt, "IB_BETA", 0.0),
    )
    setattr(model, "ib_deterministic", bool(getattr(opt, "IB_DETERMINISTIC", False)))
