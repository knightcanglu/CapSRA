...
def train_one_epoch(
    ...
    model.train()
    model.load_external_data(feature_path, neighbor_path, device)

    total_loss = total_ce = total_ib = 0.0
    correct_predictions = total_samples = 0
    batch_count = 0

    for batch_idx, (meme_ids, inputs, labels) in enumerate(
        ...
        if log_every_batches > 0 and (batch_idx + 1) % log_every_batches == 0:
            ...
            print(
                f"[Batch {batch_idx+1}/{len(dataloader)}] CE Loss: {avg_ce:.6f}, "
                f"IB Loss: {avg_ib:.6f}, Acc: {acc:.4f}"
            )
...
def evaluate(model, dataloader, device, feature_path, neighbor_path):
    model.eval()
    model.load_external_data(feature_path, neighbor_path, device)
