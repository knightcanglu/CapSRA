...
def process_split(split_name, json_path, image_dir, roi_extractor, ent_extractor,
    ...
    for item in tqdm(data, desc=f"Extracting {split_name} features", ncols=80):
        ...
        if not meme_id or not img_name:
            print(f"Warning: missing id or image field, skipping sample: {item}")
            continue
        image_path = os.path.join(image_dir, img_name)
        if not os.path.exists(image_path):
            print(f"Warning: image not found at {image_path}, skipping id={meme_id}")
            continue
        try:
            image = Image.open(image_path).convert("RGB")
        except Exception as e:
            print(f"Warning: failed to open image {image_path}: {e}, skipping id={meme_id}")
            continue
